...
export class EmployeeInvitesService {
  ...
  async create(data: Partial<EmployeeInvite> & { name: string }): Promise<{ invite: EmployeeInvite; link: string }> {
    ...
    const maxEmployees = company?.subscription_plan?.features_json?.employee_limit ?? Number.POSITIVE_INFINITY
  ...
  async findById(id: number): Promise<EmployeeInvite | null> {
